...
import { ZiroApp } from "..";
import { extend } from "../lib/extendObject";
import {
	generateBuildDirectoryFromFilename,
	isZiroPage,
} from "../lib/ziroPages";
...
export const buildServerBundles = async (app: ZiroApp) => {
	const pluginsFiles = app.thirdPartyRoutesArray
	...
	await build(
		extend(defaultBuildConfig, {
			build: {
				...
				rollupOptions: {
					input: [
						...(await glob(joinURL(process.cwd(), "pages/**/*.tsx"), {
							ignore: "node_modules/**",
						})),
						...pluginsFiles,
					],
					output: {
						entryFileNames(chunkInfo) {
							if (
								chunkInfo.facadeModuleId?.startsWith(
									joinURL(process.cwd(), "pages"),
								)
							) {
								return joinURL(
									generateBuildDirectoryFromFilename(chunkInfo.facadeModuleId),
									`${chunkInfo.name}.mjs`,
								);
							}
