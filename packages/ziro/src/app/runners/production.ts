...
import { ZiroRoute, bootstrapZiroApp, ziroDefaultConfig } from "..";
...
export const ziroProductionServer = async () => {
	...
	const manifest = readJsonFile(
		joinURL(process.cwd(), ".ziro", "server-bundles", ".vite", "manifest.json"),
	);
	...
	const routeParser = async (route: ZiroRoute) => {
		if (route.filePath) {
			...
			route.clientBundle = async () =>
				await import(
					joinURL(process.cwd(), ".ziro", "server-bundles", routeManifest.file)
				);
			route.serverBundle = async () => {
				...
				return await import(
					joinURL(process.cwd(), ".ziro", "server-bundles", pathSplit.join("/"))
				);
			};
			// @ts-ignore
			...
			clientBundle: async () =>
				await import(
					joinURL(
						...
						".ziro",
						"server-bundles",
						manifest[`pages/${paths[route]}`].file,
					)
				),
			serverBundle: async () => {
	...
	router.add(
		...
		eventHandler(async (event) => {
			...
			return readFileSync(
				joinURL(process.cwd(), ".ziro", "client-bundles", filePath),
			);
		}),
	);
