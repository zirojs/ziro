...
export const pathGenerator = (directories: Record<string, string[]>) => {
	...
	Object.keys(directories).forEach((dirPath) => {
		if (
			...
			!dirPath.startsWith(joinURL(process.cwd(), "/pages"))
		)
			return;

		directories[dirPath].map((filename) => {
			const dirPrettyPath = dirPath.replace(
				joinURL(process.cwd(), "/pages"),
				"",
			);
			if (rootRegex.test(filename))
				paths[joinURL(dirPrettyPath, "/")] = joinURL(dirPath, filename);
			if (namedRouteRegex.test(filename)) {
				const m = namedRouteRegex.exec(filename);
				const routeName = m?.[1];
				paths[joinURL(dirPrettyPath, `/:${routeName}`)] = joinURL(
					dirPath,
					filename,
				);
			}
